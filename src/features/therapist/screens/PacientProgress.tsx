...
import { Smile, Activity, Moon, Battery, TrendingUp } from "lucide-react";
...
function TherapistPacientProgress() {
    const pacients = useUserStore(useShallow(state => state.users.filter(u => u.role === "pacient")));
    const [currentPacient, setCurrentPacient] = useState<Pacient | null>(pacients[0] || null);
    const progressEntries = useProgressEntriesStore(state => state.progressEntries).filter((entry) => entry.patientId === currentPacient?.id).filter(entry => new Date(entry.date) >= new Date(new Date().getTime() - 1000 * 60 * 60 * 24 * 7)).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
    ...
    return (
        <Layout>
            <div className="flex sm:flex-row flex-col justify-between sm:items-center my-4 gap-2 sm:gap-0">
                ...
                <Combobox items={pacients} value={currentPacient} onValueChange={setCurrentPacient} itemToStringLabel={(value: Pacient) => `${value.firstName} ${value.lastName}`} >
                    <ComboboxInput placeholder="Selecione um paciente..." className="w-full sm:w-auto text-black! bg-white! border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                    <ComboboxContent>
                        <ComboboxEmpty>Nenhum paciente encontrado.</ComboboxEmpty>
                        <ComboboxList>
                            {pacients.map(p => (
                                <ComboboxItem key={p.id} value={p} className="cursor-pointer hover:bg-gray-100 px-3 py-2 rounded-md">
            ...
            <TherapistProgressChart pacient={currentPacient} />
        </Layout>
    )
}
...
function TherapistProgressChart({ pacient }: { pacient: Pacient | null }) {
    ...
    const chartData = useProgressEntriesStore(state => state.progressEntries).filter(entry => entry.patientId === pacient?.id).filter(entry => new Date(entry.date) >= new Date(new Date().getTime() - 1000 * 60 * 60 * 24 * 30)).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())

    return (
        <CustomCard>
            <CustomCardHeader>
                <div>
                    <CustomCardTitle><TrendingUp className="inline-block" /> Evolução do {pacient?.firstName}</CustomCardTitle>
                    <CustomCardSubtitle>Acompanhamento dos últimos 30 dias</CustomCardSubtitle>
                </div>
            </CustomCardHeader>
            <CustomCardContent>
...
export default TherapistPacientProgress;